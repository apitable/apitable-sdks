...
export class APITable {
  ...
  constructor(config: IAPITableClientConfig) {
    if (!config?.token) {
      throw new Error('Please pass in the API Token.(Login to apitable and get it from the user center at the bottom left corner)');
    }
      ...
      headers: {
        common: {
          ...(
            ...
            (isBrowser ? { 'X-APITable-User-Agent': 'APITableJSSDK' } : { 'User-Agent': 'APITableJSSDK' })
          ),
